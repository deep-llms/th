...
def load_model(checkpoint_path, device, dtype=None):
    ...
    has_embhub = os.path.isfile(embhub_cfg_path) and os.path.isfile(embhub_wt_path)
    ...
    if has_embhub:
        ...
        model_type = f"EmbHub (num={hub_cfg['num_embeddings']}, alpha={hub_cfg['alpha']})"
    else:
        model_type = "Baseline (no EmbHub)"
...
def main():
    ...
    model, model_type = load_model(args.checkpoint, args.device, dtype=dtype)
